    ...
    <dialog className="modal modal-open">
      <div className="modal-box bg-[#1e1e2f] text-gray-100 border border-white/10 rounded-2xl shadow-lg relative">
        ...
        <div className="flex flex-col items-center text-center space-y-3 mt-2">
          <h3 className="text-xl font-semibold text-[#f87171]">Delete Message</h3>
          <p className="text-sm text-gray-300">Are you sure you want to permanently delete this message?</p>
        </div>
        <div className="modal-action justify-center mt-6 space-x-2">
          <button onClick={props.onCancel} className="btn btn-sm bg-transparent border border-white/20 hover:bg-white/10 text-gray-300 px-6">
            <p>Cancel</p>
          </button>
          <button onClick={props.onConfirm} className="btn btn-sm bg-red-600 hover:bg-red-700 border-none text-white px-6">
            <p>Delete</p>
          </button>
        </div>
      </div>
