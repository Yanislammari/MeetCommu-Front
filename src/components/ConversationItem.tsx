...
const ConversationItem: React.FC<ConversationItemProps> = (props) => {
  const displayName =
    props.conversation.type === ConversationType.DIRECT
      ? props.otherUser?.username ?? "Unknown"
      : props.conversation.title;

  const timestamp = props.lastMessage
    ? getTimestamp(new Date(props.lastMessage.createdAt))
    : "";

  return (
    <div
      onClick={props.onSelect}
      className={`flex items-center gap-3 px-4 py-3 cursor-pointer transition-all hover:bg-[#1e1e2f] ${
        props.isSelected ? "bg-[#1e1e2f]" : ""
      }`}
    >
      {props.conversation.type === ConversationType.DIRECT && props.otherUser ? (
        ...
        <img
          src={props.conversation.pictureUrl}
          alt={displayName ?? "?"}
      ...
      <div className="flex-1">
        ...
        <p className="text-sm text-gray-400 truncate">
          {props.lastMessage?.content ?? "No messages yet"}
        </p>
      </div>
